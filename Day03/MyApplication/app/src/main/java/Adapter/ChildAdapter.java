...
public class ChildAdapter extends RecyclerView.Adapter<ChildAdapter.ChildViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ChildViewHolder holder, int position) {
        ...
        Uri gifUri = getDownloadedGifUri(context, gifUrl);

        if (gifUri != null) {
            // Load GIF đã tải từ local
            Glide.with(context)
                    .asGif()
                    .load(gifUri)
    ...
    private Uri getDownloadedGifUri(Context context, String gifUrl) {
        if (gifUrl == null || !gifUrl.contains("/")) return null;
